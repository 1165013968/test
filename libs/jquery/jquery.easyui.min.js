...
(function($) {
    ...
    function _68(_69) {
        ...
        _6c = _6c.replace(/{from}/, _6a.pageSize * (_6a.pageNumber - 1) + (_6a.total==0?0:1)); //陈峰  2012年8月31日10:03:24  修改分页显示值
    ...
    $.fn.pagination.defaults = {total:0,pageSize:10,pageNumber:1,pageList:[10,20,30,50],loading:false,buttons:null,showPageList:true,showRefresh:true,onSelectPage:function(_75, _76) {
...
(function($) {
    ...
    ;
    function _1de(_1df, _1e0, _1e1,closeFn) {
        var win = $("<div class=\"messager-body\"></div>").appendTo("body");
        win.append(_1e0);
        if (_1e1) {
            var tb = $("<div class=\"messager-button\"></div>").appendTo(win);
            for (var _1e2 in _1e1) {
//原为：$("<a></a>").attr("href","javascript:void(0)") 。author:gaofeng
                $("<a id='currentAlertButton'></a>").attr("href", "#").text(_1e2).css("margin-left", 10).bind("click", eval(_1e1[_1e2])).appendTo(tb).linkbutton();
            }
        }
        ...
        //liz修改 2012-1-11 20:10:36
        //document.getElementById("currentAlertButton").focus();//弹出框式时，"确定"按钮得到焦点。author:gaofeng
        return win;
    }

    ;
    $.messager = {show:function(_1e3) {
        var opts = $.extend({showType:"slide",showSpeed:600,width:250,height:100,msg:_1e3,title:"",timeout:4000}, _1e3 || {});
        ...
        win.window("open");
    },showext:function (_1cf) {
        var opts = $.extend({isSuccess:false, msg:"", obj:null, timeout:4000}, _1cf || {});
        var win;
        if (opts.isSuccess) {
            opts.msg = "\u64cd\u4f5c\u6210\u529f";
            win = $("<div id='msg' style='position: absolute;width:70px;height:20px;top:2px;right:18px;background-color: green; border: 1px solid gray; color: white;padding-top:2px;'></div>").html(opts.msg).appendTo(opts.obj);
        } else {
            opts.msg = "\u64cd\u4f5c\u5931\u8d25";
            win = $("<div id='msg' style='position: absolute;width:70px;height:20px;top:2px;right:18px;background-color: red; border: 1px solid gray; color: black;padding-top:2px;'></div>").html(opts.msg).appendTo(opts.obj);
        }
        setTimeout("$('#msg').fadeOut('slow',function(){$('#msg').remove();})", opts.timeout);
    }, alert:function(_1e4, msg, icon, fn,closeFn) {
        var _1e5 = "<div class='messager-cont'>" + msg + "</div>";
        // switch (icon) {
        ...
        var win = _1de(_1e4, _1e5, _1e6, closeFn);
    },confirm:function(_1e7, msg, fn,type) {
        var _1e8 = "<div class=\"messager-icon messager-question\"></div>" + "<div class='messager-cont'>" + msg + "</div>" + "<div style=\"clear:both;\"/>";
        var _1e9 = {};
        //根据变量来选择按钮的样式 add by hzl 
        var tempY = $.messager.defaults.ok;
        var tempN = $.messager.defaults.cancel;
        if(type=='y') {
        	tempY = $.messager.defaults.yes;
        	tempN = $.messager.defaults.no;
        }
        _1e9[tempY] = function() {
            win.window("close");
            if (fn) {
        ...
        var _1eb = "<div class=\"messager-icon messager-question\"></div>" + "<div class='messager-cont'>" + msg + "</div>"  + "<input class=\"messager-input\" type=\"text\"/>" + "<div style=\"clear:both;\"/>";
        var _1ec = {};
    ...
    },autoHideAlert:function(title,msg,icon){//add by xiaoting  添加自动隐藏提示框
    	 var opts = $.extend({showType:"fade",showSpeed:100,width:250,height:"auto",msg:msg,title:title,timeout:1000}, msg || {});
    	 var win = $("<div class=\"messager-body\"></div>").appendTo("body");       
    	 var _1e5 = "<div class='messager-cont'>" + msg + "</div>";
        //  switch (icon) {
        //      case "error":
        ...
        //  }
         ...
         var tb = $("<div class=\"messager-button\"></div>").appendTo(win);
         var funs = {};
         funs[$.messager.defaults.ok] = function() {
             win.dialog({closed:true});
         };
         for (var fun in funs) {
        	       $("<a id='currentAlertButton'></a>").attr("href", "#").text(fun).css("margin-left", 10).bind("click", eval(funs[fun])).appendTo(tb).linkbutton();
         }
         win.window({title:opts.title,width:opts.width,height:opts.height,collapsible:false,minimizable:false,maximizable:false,shadow:false,draggable:false,resizable:false,closed:true,onBeforeOpen:function() {
                     show(this, opts.showType, opts.showSpeed, opts.timeout);
                     return false;
         ...
         win.window("window").css({zIndex:$.fn.window.defaults.zIndex++,bottom:-document.body.scrollTop - document.documentElement.scrollTop});
         win.window("open");
    },autoWidthAndHeightAlert:function(title,msg,icon,wid,hei){//add by xiaoting  自动设置alert框的大小
        var opts = $.extend({showType:"fade",showSpeed:100,width:wid?wid:'auto',height:hei?hei:'auto',msg:msg,title:title,timeout:0}, msg || {});
        var win = $("<div class=\"messager-body\"></div>").appendTo("body");
        var _1e5 = "<div class='messager-cont'>" + msg + "</div>";
        // switch (icon) {
        //     case "error":
    ...
    $.messager.defaults = {ok:"确定",cancel:"取消",yes:"是",no:"否"};//提示按钮改为中文，author:gaofeng
})(jQuery);
(function($) {
    function _1ed(_1ee) {
...
(function($) {
    ...
    function _243(_244) {
        ...
        for (var i = 0,len = tabs.length; i < len; i++) {
            ...
            tab.find("a.tabs-close").unbind(".tabs").bind("click.tabs", {title:_248}, function(e) {
                ...
                /**
                 *Tab关闭时，如果数据已修改，提示用户是否强制离开
                 *author:gaofeng
                 *data:2011-9-16
                 */
                var tabs = $.data(_244, "tabs").tabs;
                if (typeof(document.getElementById(e.data.title + "_tab").contentWindow.isCheckFormDataByUpdate) != "undefined") {
                    if (document.getElementById(e.data.title + "_tab").contentWindow.checkFormUpdate()) {
                        $.messager.confirm('提示', '数据已经被修改，页面离开后数据不会被保存，是否离开当前页面?', function(r) {
                            if (!r) {
                                return false;
                            } else {
    ...
    function _254(_266, _267) {
        ...
        _269.panel("open");
        //add by ych
        //点击标题时调用iframe中的__tabsClick方法
        try{
            var iframes = document.getElementsByTagName("iframe");
            for(var mm=0;mm<iframes.length;mm++){
                try{
                    iframes[mm].contentWindow.__tabsClick(_267);
                }catch(e){}
            }
        }catch(e){}
...
(function($) {
    ...
    function _2ec(_2ed) {
        ...
        alertList[alertList.length] = "【 " + box.parent().prev().html() + "】 " + msg + "!"; //增加alert提示信息 author:gaofeng
...
//tip.css({display:"block",left:box.offset().left+box.outerWidth(),top:box.offset().top});
        ...
        if (bw < (box.offset().left + box.outerWidth() + tip.width())) {
            tip.css({display:"inline", right:0, top:box.offset().top});
        } else {
            tip.css({display:"inline", left:box.offset().left + box.outerWidth(), top:box.offset().top});
        }
    }
    ...
    function _2ef(_2f0) {
        ...
        var box = $(_2f0);
        if (box.attr("readonly"))return false; //只读的文本框不校验，author:gaofeng
        var _2f1 = box.val();
         ...
         *增加分块验证变量
         *author:gaofeng
         *date:2011-9-02 19:46:00
         */
        var isCheck = true;
        var subNum = "-1";
        if (typeof(box.attr("subNum")) != 'undefined') {
            subNum = box.attr("subNum");
        }
        /**end**/
        ...
        if (opts.required) {
             ...
             *date:2011-9-02 19:46:00
             */
            if ((opts.validType != null ||opts.validType ==undefined) && validate_subNum != "") {
                if (subNum != validate_subNum) {
                    isCheck = false;
                }
            }
        ...
        if (opts.validType) {
            /*增加组合校验，用@分开，例如：email@leng[11,12]
             *author:gaofeng
             */
            ...
            for (_iii = 0; _iii < _b_validTypes.length; _iii++) {
                ...
                if (_2f1 && rule) {
                     ...
                     *date:2011-9-02 19:46:00
                     */
                    if ((opts.validType != null ||opts.validType ==undefined) != null && validate_subNum != "") {
                        if (subNum != validate_subNum) {
                            isCheck = false;
                        }
                    }
                    ...
                    if (!rule["validator"](_2f1, _2f5) && isCheck) {
                        box.addClass("validatebox-invalid");
...
(function($) {
    ...
    $.fn.form = function(_314, _315) {
        /**根据form("","1")第二个参数传入的值，设置子模块标识
         *date:2011-9-02 19:35:30
         *author: gaofeng
         */
    
        alertList = new Array();
        if (typeof(_315) != 'undefined') {
            validate_subNum = _315;
        ...
        if (typeof _314 == "string") {
            var flagValue = $.fn.form.methods[_314](this, _315);
            /**
             *未校验通过的信息用alert方式提示
             */
            if (validate_subNum == "-1") {
                var alertInfo = "";
                for (var i = 0; i < alertList.length - 1; i++) {
                    alertInfo = alertInfo + "<br>" + alertList[i];
                    if (i == 0) {
                        alertInfo = alertInfo + "<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color=red>......</font>";
                        break;
                    }
                }
                if (alertInfo != "") {
                    $.messager.alert("提示", alertInfo, 'warning');
                }
            }
            /**end**/
			 validate_subNum = ""; //重新设置空值，author:gaofeng
