...
(function($) {
	...
	var ZTREE_REASYNC_SUCCESS = "ZTREE_REASYNC_SUCCESS";
	...
	function bindTreeNodes(setting, treeObj) {
		...
		treeObj.unbind(ZTREE_REASYNC_SUCCESS);
		treeObj.bind(ZTREE_REASYNC_SUCCESS, function (event, treeId, treeNode, msg) {
		  if ((typeof setting.callback.reasyncSuccess) == "function") setting.callback.reasyncSuccess(event, treeId, treeNode, msg);
		});
	...
	function showTree(setting, treeNode) {
		...
		//显示CheckBox Or Radio
		if (setting.checkable&&treeNode.checkbox!="false") {
		...
		setNodeIdAndClass(treeNode);
		setNodeAutoExpand(treeNode);
		setNodeRootNo(treeNode);
		setNodeCheckDisabled(treeNode);
	...
	//设置Id and Class
	function setNodeIdAndClass(treeNode) {
		var aObj = $("#" + treeNode.tId + IDMark_A);
		aObj.attr("name", (treeNode.idAndClass || ""));
	}
	//设置autoExpand
	function setNodeAutoExpand(treeNode) {
		var aObj = $("#" + treeNode.tId);
		if(treeNode.autoExpand){
			aObj.attr("name", "autoExpandNode");
		}
	}
	//设置disabled
	function setNodeCheckDisabled(treeNode) {
		var aObj = $("#" + treeNode.tId + IDMark_Check);
		if(treeNode.disabled) {
			aObj.attr("disabled", "disabled");
		}
	}
	//设置rootNo
	function setNodeRootNo(treeNode) {
		var aObj = $("#" + treeNode.tId);
		if(treeNode.parentNode) {
			treeNode.rootNo = treeNode.parentNode.rootNo;
			aObj.attr("rootNo", (treeNode.parentNode.rootNo || "1"));
		} else if(treeNode.rootNo==null){
			treeNode.rootNo="1";
		}
	}
	...
	function setNodeUrl(setting, treeNode) {
		var aObj = $("#" + treeNode.tId + IDMark_A);
		if (treeNode.url) aObj.attr("href", treeNode.url);
	...
	function replaceIcon(obj, newName,treeNode) {
		if (!obj) return;
		var tmpName = obj.attr("style");
		if (tmpName == undefined) return;
		var tempstr = "";
		switch (newName) {
		case FolderMark_Open:
			{if(treeNode.openIcon) 
				  tempstr="background:url(" + treeNode.openIcon + ") 0 0 no-repeat;";
			else tempstr="background:url(" + treeNode.icon + ") 0 0 no-repeat;";
			break;}
		case FolderMark_Close:tempstr="background:url(" + treeNode.icon + ") 0 0 no-repeat;";break;
		case FolderMark_Docu:tempstr="background:url(" + treeNode.icon + ") 0 0 no-repeat;";
		}
		obj.attr("style", tempstr);
	}
	...
	function checkNodeRelation(setting, treeNode) {
		if (setting.checkStyle == Check_Style_Radio) {
			...
			if (!treeNode[setting.checkedCol] && setting.checkType.N.indexOf("p") > -1) {
				...
				repairParentNotChk(setting, treeNode.parentNode);
			}
		}
	}
	...
	function repairParentNotChk(setting, treeNode) {
		if (!treeNode) return;
		var trueSign = true;
		var falseSign = true;
		treeNode[setting.checkedCol] = false;
		if (treeNode[setting.nodesCol]) {
			for (var son = 0; son < treeNode[setting.nodesCol].length; son++) {
				if (setting.checkStyle == Check_Style_Radio && (treeNode[setting.nodesCol][son][setting.checkedCol] || !treeNode[setting.nodesCol][son].check_True_Full)) {
					trueSign = false;
	...
	function asyncGetNodeClosed(setting, treeNode) {
		if (treeNode && (treeNode.isAjaxing || !treeNode.isParent)) {
			return;
		}
		if (treeNode) {
		...
		$.ajax({
			...
			success: function(msg) {
				...
				if (treeNode) treeNode.isAjaxing = undefined;
				expandCollapseSonNode(setting, treeNode, false, false);
				//$("#" + setting.treeObjId).trigger(ZTREE_REASYNC_SUCCESS, [setting.treeObjId, treeNode, msg]);
	...
	function expandAndCollapseNode(setting, treeNode, expandSign, animateSign, callback) {
		...
		if (treeNode.isParent && treeNode[setting.nodesCol] && treeNode[setting.nodesCol].length > 0) {
			if (!treeNode.open) {
				...
				replaceIcon(icoObj, FolderMark_Open,treeNode);
				treeNode.open = true;
				...
				replaceIcon(icoObj, FolderMark_Close,treeNode);
				treeNode.open = false;
				if (animateSign == false) {
					ulObj.hide();
					if (typeof callback == "function") callback();
	...
	function expandCollapseParentNodes(setting, treeNode, expandSign, animateSign, callback) {
		//针对动画进行优化,一般来说只有在第一层的时候，才进行动画效果
		if (!treeNode) return;
		if (!treeNode.parentNode) {
			//保证callback只执行一次
		...
		if (treeNode.parentNode) {
			expandCollapseParentNodes(setting, treeNode.parentNode, expandSign, animateSign, callback);
		}
	}
	...
	//遍历父节点设置checkbox not checked
	function setParentNodeCheckBoxNotChecked(setting, treeNode, value) {
		var checkObj = $("#" + treeNode.tId + IDMark_Check);
		treeNode[setting.checkedCol] = value;
		setChkClassNotChecked(setting, checkObj, treeNode);
		if (treeNode.parentNode) {
			var pSign = true;
			if (!value) {
				for (var son = 0; son < treeNode.parentNode[setting.nodesCol].length; son++) {
			...
			if (pSign) {
				setParentNodeCheckBoxNotChecked(setting, treeNode.parentNode, value);
			}
		}
	}
	...
	function addInit(setting, treeNode) {
		$("#" + treeNode.tId + IDMark_Span).html("<input type=text class='rename' id='" + treeNode.tId + IDMark_Input + "'>");
		var inputObj = $("#" + treeNode.tId + IDMark_Input);
		inputObj.attr("value", treeNode[setting.nameCol]);
		inputObj.focus();
	...
	function zTreePlugin(){
		return {
			...
			addInit : function(treeNode) {
				if (!treeNode) return;
				addInit(this.setting, treeNode);
			},
			
			selectEditNode : function(treeNode) {
				if (!treeNode) return;
				selectNode(this.setting, treeNode);
				//如果选择某节点，则必须展开其全部父节点
				//多个图层同时进行动画，导致产生的延迟很难用代码准确捕获动画最终结束时间
				//因此为了保证准确将节点focus进行定位，则对于js操作节点时，不进行动画
				expandCollapseParentNodes(this.setting, treeNode, true, false, function() {
					$("#" + treeNode.tId + IDMark_Icon).focus().blur();
				});
			},
			...
			reAsyncChildNodes : function(parentNode, reloadType,isOpen) {
				...
				if(isOpen) {
					...
					asyncGetNodeClosed(this.setting, isRoot? null:parentNode);
				}
