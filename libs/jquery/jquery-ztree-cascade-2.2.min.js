(function ($) {
    ...
    function bindTreeNodes(setting, treeObj) {
        ...
        $("#" + setting.treeObjId).bind('contextmenu', function (event) {
        ...
        $("#" + setting.treeObjId).bind('mouseup', function (event) {
            var targetObj = $(event.target);
            var treeNode = getTreeNodeByDom(setting, targetObj);
            var doMouseUp = true;
        ...
        $("#" + setting.treeObjId).bind('mousedown', function (event) {
            var targetObj = $(event.target);
            var treeNode = getTreeNodeByDom(setting, targetObj);
    ...
    function showTree(setting, treeNode) {
        ...
        ulObj.css({"display": (treeNode.open ? "block" : "none")});
        if (treeNode.isParent) {
            switchObj.bind('click', {treeObjId: setting.treeObjId, treeNode: treeNode}, onSwitchNode);
            aObj.bind('dblclick', {treeObjId: setting.treeObjId, treeNode: treeNode}, onSwitchNode);
        }
        ...
        if (setting.checkable && treeNode.checkbox != "false") {
            ...
            checkObj.bind('click', function () {
                ...
                if (setting.async && setting.asyncUrl && setting.asyncUrl.length > 0) {
                    if(treeNode.checkbox){
                        if(treeNode.isParent&&null==treeNode[setting.nodesCol]){
                            asyncGetNodeSelect(setting,treeNode);
                        }
                    }
                }
        ...
        aObj.bind('mousedown', function (eventMouseDown) {
            ...
            $(doc).mousemove(function (event) {
                ...
                window.getSelection ? window.getSelection().removeAllRanges() : document.selection.empty();
                if (setting.dragStatus == 0 && Math.abs(mouseDownX - event.clientX) < MinMoveSize && Math.abs(mouseDownY - event.clientY) < MinMoveSize) {
                    return true;
                }
                ...
                if (setting.dragStatus == 1 && tmpArrow.attr("id") != event.target.id) {
                    ...
                    curNode.css({"top": (event.clientY + docScrollTop + 3) + "px", "left": (event.clientX + docScrollLeft + 3) + "px"});
                    var dX = 0;
                    var dY = 0;
                    if (tmpTarget && tmpTarget.attr("id") != setting.treeObjId) {
                        ...
                        tmpArrow.css({"display": "block", "top": (tmpTargetA.offset().top + dY) + "px", "left": (tmpTargetA.offset().left + dX) + "px"});
    ...
    function setNodeUrl(setting, treeNode) {
        var aObj = $("#" + treeNode.tId + IDMark_A);
        if (treeNode.url)aObj.attr("href", treeNode.url); else aObj.removeAttr("href");
    }
        ...
        switch (newName) {
            case FolderMark_Open:
            {
                if (treeNode.openIcon)
                    tempstr = "background:url(" + treeNode.openIcon + ") 0 0 no-repeat;"; else tempstr = "background:url(" + treeNode.icon + ") 0 0 no-repeat;";
                break;
            }
            case FolderMark_Close:
                tempstr = "background:url(" + treeNode.icon + ") 0 0 no-repeat;";
                break;
            case FolderMark_Docu:
                tempstr = "background:url(" + treeNode.icon + ") 0 0 no-repeat;";
        }
    ...
    function asyncGetNode(setting, treeNode) {
        ...
        $.ajax({type: "POST", url: url, data: tmpParam, success: function (msg) {
            if (!msg || msg.length == 0) {
                return;
            }
            var newNodes = "";
    ...
    function asyncGetNodeSelect(setting, treeNode) {
        if (treeNode && (treeNode.isAjaxing || !treeNode.isParent)) {
            return;
        }
        if (treeNode) {
        ...
        $.ajax({type: "POST", url: url, data: tmpParam, success: function (msg) {
            ...
            $("#" + setting.treeObjId).trigger(ZTREE_ASYNC_SUCCESS, [setting.treeObjId, treeNode, msg]);
            if (treeNode[setting.checkedCol] && setting.checkType.Y.indexOf("s") > -1) {
                setSonNodeCheckBoxSelect(setting, treeNode, true);
                repairSonChkClass(setting, treeNode);
            }
            if (treeNode[setting.checkedCol] && setting.checkType.Y.indexOf("p") > -1) {
                setParentNodeCheckBox(setting, treeNode, true);
            ...
            if (!treeNode[setting.checkedCol] && setting.checkType.N.indexOf("s") > -1) {
                setSonNodeCheckBoxSelect(setting, treeNode, false);
                repairSonChkClass(setting, treeNode);
            }
            if (!treeNode[setting.checkedCol] && setting.checkType.N.indexOf("p") > -1) {
                setParentNodeCheckBox(setting, treeNode, false);
    ...
    function setSonNodeCheckBox(setting, treeNode, value) {
        ...
        for (var son = 0; son < treeNode[setting.nodesCol].length; son++) {
            if(treeNode[setting.nodesCol][son].isParent&&null==treeNode[setting.nodesCol][son][setting.nodesCol]){
                asyncGetNodeSelect(setting,treeNode[setting.nodesCol][son]);
            }
            if (treeNode[setting.nodesCol][son])setSonNodeCheckBox(setting, treeNode[setting.nodesCol][son], value);
        }
    }

    function setSonNodeCheckBoxSelect(setting, treeNode, value) {
        if (!treeNode)return;
        var checkObj = $("#" + treeNode.tId + IDMark_Check);
        if (treeNode != setting.root) {
            treeNode[setting.checkedCol] = value;
    ...
    function addTreeNodes(setting, parentNode, newNodes, isSilent) {
        ...
        if (parentNode) {
            ...
            if (!parentNode.isParent) {
                ...
                target_aObj.bind('dblclick', {treeObjId: setting.treeObjId, treeNode: parentNode}, onSwitchNode);
            }
    ...
    function moveTreeNode(setting, targetNode, treeNode, moveType, animateSign) {
        ...
        if (moveType == MoveType_Inner) {
            ...
            if (!targetNode.isParent && !targetNodeIsRoot) {
                ...
                target_aObj.unbind('dblclick');
                target_aObj.bind('dblclick', {treeObjId: setting.treeObjId, treeNode: targetNode}, onSwitchNode);
            }
    ...
    function zTreePlugin() {
        return{container: null, setting: null, init: function (obj) {
            this.container = obj;
